...
const Navbar = () => {
  ...
  const handleNavClick = (e, item) => {
    ...
    if (item === "Research" || item === "Blogs") {
      setNotice(`${item} section is currently under construction.`);
  ...
  return (
    <>
      ...
      {notice && (
        <div className="fixed top-24 left-1/2 transform -translate-x-1/2 z-50 bg-black text-white px-6 py-3 rounded-full text-sm font-medium shadow-lg transition-all animate-fadeIn">
          {notice}
        </div>
      )}
    </>
        ...
        <div className="relative w-full h-[60vh] md:h-[75vh] min-h-[500px] bg-black rounded-[2rem] md:rounded-[3rem] overflow-hidden flex flex-col items-center justify-center border border-black/5">
          ...
          <div className="relative z-10 flex flex-col items-center text-center max-w-3xl px-6">
            ...
            <button 
              ...
              className="bg-white text-black px-8 py-3 rounded-full font-medium text-sm transition-colors duration-300 hover:bg-gray-200 cursor-pointer"
            >
              Learn More
            </button>
          </div>
...
const Careers = () => {
  ...
  return (
    <section id="careers" className="py-24 px-6 md:px-12 max-w-7xl mx-auto w-full border-t border-black/10">
      <FadeInSection>
        <h2 className="text-sm font-bold tracking-widest uppercase mb-4 text-black">
          Careers
        </h2>
        <h3 className="text-3xl md:text-4xl font-bold text-black tracking-tight mb-12">
          Open Positions
        </h3>
        ...
        <div className="grid grid-cols-1 gap-6">
          {positions.map((pos, index) => (
            <FadeInSection key={index} delay={index * 100}>
              <div className="bg-white border border-gray-200 p-8 md:p-10 rounded-[1.5rem] flex flex-col md:flex-row justify-between items-start md:items-center gap-6 transition-all duration-300 hover:border-black/30 hover:shadow-sm">
                <div className="max-w-2xl">
                  <div className="flex flex-wrap items-center gap-3 mb-2">
                    <h4 className="text-xl font-bold text-black tracking-wide">
                      {pos.title}
                    </h4>
                    <span className="text-xs font-bold px-3 py-1 bg-[#F5F5F5] rounded-full text-black/70">
                      {pos.location}
                    </span>
                  </div>
                  <p className="text-black/70 text-base font-medium mt-2">
                    <strong className="text-black font-semibold">Requirements:</strong> {pos.requirements}
                  </p>
                </div>
                <button
                  onClick={() => handleApply(pos.title, pos.location)}
                  className="bg-black text-white px-6 py-3 rounded-full font-medium text-sm transition-colors duration-300 hover:bg-black/80 whitespace-nowrap shrink-0 cursor-pointer"
                >
                  Submit Profile
                </button>
              </div>
...
const Footer = () => {
  ...
  return (
    <footer id="footer" className="bg-white border-t border-black/10 pt-24 pb-12 px-6 md:px-12 w-full mt-12">
