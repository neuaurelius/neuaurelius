...
const Navbar = () => {
  ...
  useEffect(() => {
    const sections = ["research", "careers"];
  ...
  return (
    ...
    >
      <div className="max-w-[1200px] mx-auto px-5 sm:px-8 flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center gap-2 p-1 bg-[#f5f5f7] rounded-full border border-[#e5e5ea]">
          {navItems.map((item) => {
        ...
        <button 
          className="md:hidden z-50 p-3 -mr-2 text-[#1d1d1f] bg-[#f5f5f7] hover:bg-[#e5e5ea] rounded-full transition-all duration-300 cursor-pointer active:scale-95 group shadow-sm"
          onClick={() => setMobileMenuOpen(!mobileMenuOpen)}
          aria-label="Toggle Menu"
          aria-expanded={mobileMenuOpen}
        >
          <div className="w-5 h-4 relative flex flex-col justify-between">
            <span className={`w-full h-[2px] bg-current rounded-full transition-all duration-300 origin-center ${mobileMenuOpen ? 'rotate-45 translate-y-[7px]' : 'group-hover:w-4'}`} />
            <span className={`w-full h-[2px] bg-current rounded-full transition-all duration-300 ${mobileMenuOpen ? 'opacity-0 scale-x-0' : ''}`} />
            <span className={`w-full h-[2px] bg-current rounded-full transition-all duration-300 origin-center ${mobileMenuOpen ? '-rotate-45 -translate-y-[7px]' : 'group-hover:w-3'}`} />
          </div>
        </button>

      </div>

      {/* Solid Opaque Mobile Menu Overlay with Smooth Staggered Animation */}
      <div 
        className={`fixed inset-0 bg-white z-40 transition-all duration-500 ease-[cubic-bezier(0.16,1,0.3,1)] md:hidden flex flex-col justify-between pt-28 pb-12 px-6 sm:px-8 ${
    ...
    <section className="relative w-full h-[75vh] sm:h-[calc(80vh-2rem)] min-h-[500px] flex items-center justify-center pt-14 bg-white overflow-hidden">
      <div className="w-full h-full max-w-[1920px] mx-auto transition-all duration-300 ease-out flex items-center justify-center" style={containerStyle}>
        <div className="relative w-full h-full overflow-hidden transition-all duration-300 ease-out bg-[#1d1d1f] shadow-[0_24px_80px_rgba(0,0,0,0.08)]" style={innerStyle}>
          ...
          <div className={`absolute inset-0 bg-gradient-to-br from-[#2c2c2e] to-[#111113] transition-opacity duration-1000 ${imageLoaded || imageError ? 'opacity-0' : 'opacity-100'}`}>
            <div className="w-full h-full flex flex-col items-center justify-center gap-3 text-[#a1a1a6] text-xs sm:text-sm tracking-wide">
              <div className="w-6 h-6 border-2 border-white/20 border-t-white rounded-full animate-spin"></div>
              <span>Optimizing for connection...</span>
            </div>
          </div>
...
const Team = () => {
  ...
  return (
    <section className="pt-8 pb-24 sm:pb-32 bg-white px-5 sm:px-8">
      <div className="max-w-[1200px] mx-auto">
        <FadeInSection>
